...
public struct Button : View, Renderable {
    ...
    @Composable static func RenderButton(label: View, context: ComposeContext, role: ButtonRole? = nil, isEnabled: Bool = EnvironmentValues.shared.isEnabled, action: () -> Void) {
        ...
        ComposeContainer(modifier: context.modifier) { modifier in
            ...
            case is GlassButtonStyle, is GlassProminentButtonStyle: // Liquid Glass: see Button+LiquidGlass.swift
                RenderGlassButton(label: label, context: context.content(modifier: modifier), role: role, isEnabled: isEnabled, isProminent: buttonStyle is GlassProminentButtonStyle, action: action)
            default:
...
/// Liquid Glass: see Button+LiquidGlass.swift
...
extension View {
    ...
    public func buttonStyle(bridgedStyle: Int) -> any View {
        ...
        case 5:
            return buttonStyle(GlassButtonStyle())
        case 6:
            return buttonStyle(GlassProminentButtonStyle())
        case 7:
...
final class StackedButtonStyle {
    ...
    var isBuiltin: Bool {
        return style is DefaultButtonStyle || style is PlainButtonStyle || style is BorderlessButtonStyle || style is BorderedButtonStyle || style is BorderedProminentButtonStyle || style is M3TextButtonStyle || style is GlassButtonStyle || style is GlassProminentButtonStyle
    }
}
